...
config/secondmate-pins/<id>  optional per-secondmate model/effort override keyed by registry id; LOCAL, gitignored; a pinned axis overrides the matching config/secondmate-harness token for that one secondmate while every unpinned axis and other secondmate keep the global fallback, and it resolves durably on every spawn and relaunch. The primary's own config; NOT inherited into secondmate homes. Schema and validation owned by docs/configuration.md "config/secondmate-pins"
