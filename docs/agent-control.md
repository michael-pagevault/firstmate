...
1. **Resolve the profile.**
   ...
   Otherwise a `kind=secondmate` task re-resolves its durable configured pin - its harness and the fallback model/effort tokens from `config/secondmate-harness`, and a per-secondmate model/effort from `config/secondmate-pins/<id>` when present - exactly as every other respawn does, so setting the pin and relaunching is the ordinary way to move a secondmate's runtime.
