...
- A Charted Next row's optional `kind` separates work from alarms: omit it (or set `"queued"`) for real queued work, and set `"warning"` on every action-free fleet-integrity notice - the `(main-inventory)` gate, an unavailable secondmate home, and an inventory-mismatch repair notice. The board badges each row by category so it reads at a glance: a `warning` row shows `needs repair`, a dispatchable queued row with no `reason` shows `ready`, and every other queued row (one carrying a `reason`, or one not yet dispatchable) shows `waiting`; a warning is also left out of the Charted Next count, so those rows never read as dispatchable queued work.
- A Recently Landed row's optional `pr_url` is the board's landed category: a row carrying it is badged `merged PR` (shipped code) and every other row `completed` (a finished scout, investigation, or local-only merge), so set `pr_url` only on a row whose work actually merged as a PR.
- The optional `updates` array is the Meaningful Updates card stream: substantive report or milestone outcomes that help the captain resume, composed from the same snapshot with the same ranking judgment, separate from Captain's Call and the operational sections and carrying no answer control.
  Surface only substantive outcomes - a completed investigation from `reports`, a material validation finding, a newly surfaced blocker, a decision the captain just resolved, a PR-ready outcome, or another milestone.
  Exclude routine progress, repeated or unchanged status text, still-waiting states, stale-pane noise, and any update materially identical to one already shown.
  Each card carries an explicit `repo` (filled from the snapshot and task records wherever known, null or empty only as the genuinely-no-repo marker), a nonempty `id`, a plain-language `headline`, a `category` (one of `investigation`, `validation`, `blocker`, `decision`, `pr-ready`, `milestone`), and optionally a one-line `detail` and an https `pr_url`; keep `detail` a distilled sentence, never a raw status-log dump, and never include secret values.
  `updates_more` counts omitted update cards when the stream truncates.
  Meaningful Updates is a board-only surface: it does not change the four-section chat digest or the meanings of Captain's Call, Underway, Recently Landed, or Charted Next.
